  ...
  $anch.each(function() {
    ...
    function onTabClick(_obj, _numm) {
      ...
      $anch
        ...
        .not(_this)
        .next()
        .css('display', 'none'); //클릭했을때 다음에있는 object block 나머지는 none
      $anch_img.chgImg(0);
      _this_img.chgImg(1);
  ...
  jQuery(this).each(function() {
    ...
    if (on) {
      if (chkOn) {
        ...
        jQuery(this).attr(
          'src',
          imgSrc.replace(settings.imgType, settings.replaceTxt)
        );
      }
    } else if (chkOn) {
      jQuery(this).attr(
        'src',
        imgSrc.replace(settings.replaceTxt, settings.imgType)
      );
    }
  });
};
...
function columResize() {
  if (
    ...
    jQuery('.jQFamily_btn').click(function() {
      jQuery('.family_link_wrap').css('display', 'block');
      jQuery('.family_link_wrap')
        .stop()
        .animate({ height: '474px' }, 300, 'easeOutExpo', function() {});
    });
    jQuery('.jQFamily_close_btn').click(function() {
      jQuery('.family_link_wrap')
        .stop()
        .animate({ height: '0' }, 300, 'easeOutExpo', function() {
          jQuery('.family_link_wrap').css('display', 'none');
        });
    });
