jQuery(document).ready(function(){
	if(navigator.userAgent.match(/Android|Mobile|iPhone|iPad|iPod|BlackBerry|IEMobile|Kindle|NetFront|Silk-Accelerated|(hpw|web)OS|Fennec|Minimo|Opera M(obi|ini)|Blazer|Dolfin|Dolphin|Skyfire|Zune/i)){
		...
		jQuery('.gnb').mouseenter(function(){
			...
			jQuery('.gnb_bg').stop().animate({'height': '420px'}, 300, "easeOutExpo", function() {

			});
			$gnbDep2.stop().animate({'height': '420px'}, 300, "easeOutExpo", function() {
	...
	/*jQuery('.jQmomenuon').click(function(){
		jQuery('.menu_side').css({'width':'80%' , 'display':'block'});
		jQuery('.padhide').css({'display':'block'});
		jQuery('.black_bg').css({'display':'block'});
		jQuery(this).addClass('active');
		jQuery('.contents_wrap').css('position','fixed');
	});
	jQuery('.jQmomenuoff').click(function(){
	...
	// menu show / hide
	jQuery('.jQmenuon').click(function(){
		jQuery('.menu_side').css({'width':'325px'});
		jQuery('.padhide').css({'display':'block'});
		jQuery('.jQmenuoff').css({'display':'block'});
		jQuery(this).css('display','none');
		jQuery(this).addClass('active');
	});
	jQuery('.jQmenuoff').click(function(){
		jQuery('.menu_side').css({'width':'77px'});
		jQuery('.padhide').css({'display':'none'});
		jQuery('.jQmenuon').css({'display':'block'});
		jQuery(this).css('display','none');
	});*/
});
...
function GNB(){
	...
	function subOn(el){
		...
		el.parent().parent().parent().prev().addClass('active'); 
	}
